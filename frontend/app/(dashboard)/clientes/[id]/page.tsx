...
import { buscarCliente } from "@/app/servicos/clientes"
import { buscarScore } from "@/app/servicos/ia"
import { listarPedidos } from "@/app/servicos/pedidos"
...
import type { ScoreCliente } from "@/app/tipos"
...
export default function PaginaCliente({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { dados, carregando, erro, recarregar } = useCarregar(async () => {
    ...
    let score: ScoreCliente | null = null
    try { score = await buscarScore(clienteId) } catch { score = null }
    return { cliente, score, pedidos: pedidos.filter(p => p.clienteId === clienteId), produtos }
  }, [clienteId])
  ...
  return (
    <div className="p-8 animar-entrada">
      ...
      <EstadoConteudo carregando={carregando} erro={erro} aoTentar={recarregar}>
        {dados && (() => {
          const { cliente, score } = dados
          const itensPedidos = dados.pedidos.map(p => ({
            id: `#${String(p.id).padStart(4, "0")}`,
            itens: p.itens.map(i => dados.produtos.find(pr => pr.id === i.produtoId)?.nome ?? "-").join(", "),
          ...
          return (
            <>
              ...
              <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
                <CardStat rotulo="Total de Pedidos" valor={String(score?.totalPedidos ?? dados.pedidos.length)} />
