...
const sortear = (chance) => aleatorio() < chance;
const HOJE = new Date();
...
function arquetipoDe(indice) {
    const fracao = indice / 500;
    if (fracao < 0.2)
        return ARQUETIPOS.fiel;
    if (fracao < 0.55)
        return ARQUETIPOS.regular;
    if (fracao < 0.85)
        return ARQUETIPOS.emRisco;
...
function gerarClientes() {
    ...
    for (let i = 0; i < 500; i++) {
        ...
        while (usados.has(email))
            email = `${nome.toLowerCase().replace(/ /g, '.')}${i}.${inteiro(1, 999)}@beast-mail.com`;
        usados.add(email);
...
async function criarClientes(clientes) {
...
async function criarPedidos(clientes, ids, produtos) {
    const pedidos = [];
    ...
    const itens = criados.flatMap((pedido, indice) => itensPorPedido[indice].map((item) => ({ pedidoId: pedido.id, ...item })));
...
//# sourceMappingURL=povoar.js.map